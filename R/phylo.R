...
cisort <- function(x,y){
    k = length(x)
    l=length(y)
    .C("C_cisort",as.integer(x),as.integer(y),k,l, integer(k+l), DUP=FALSE)[[5]]
}
...
# needs to be more elegant pa changed for dad           
                if(loli == rootNode && dad!= loli){                    
#                    browser() # update all nodes
